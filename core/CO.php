...
class CO
{
    ...
    public function handle($app_name)
    {
        $pipeline = new Pipeline();
        $segments = self::Router();
        //解析出控制器和函数
        $ctrl_name = empty($segments[1]) ? self::DEFAULT_CONTROLLER : $segments[1];
        $func_name = empty($segments[2]) ? self::DEFAULT_FUNCTION : $segments[2];
        ...
        if (is_callable(array(array('CO_Controller', $func_name)))) {
        ...
        return $pipeline->send($objCtrl)
                    ->throuth($this->middleware)
                    ->then(function ($objCtrl) use ($app_name,$controller,$func_name) {
                        if (method_exists($objCtrl, $func_name)) {
                            ...
                            if ('public' != strtolower($r[0])) {
                                //非public方法不能调用
                                unset($r);
                                unset($ref);

                                return $objCtrl->output;
                            }
                            $ret = $objCtrl->$func_name();    //运行
                            $objCtrl->output->appendOutputStream($ret);
                        }
    ...
    public function route($app_name, $input)
    {
        $segments = self::Router();
        //解析出控制器和函数
        $ctrl_name = empty($segments[1]) ? self::DEFAULT_CONTROLLER : $segments[1];
        ...
        include $controller_file_path;    //引入控制器文件
        $objCtrl = new $controller($app_name, $input);
        if (method_exists($objCtrl, $func_name)) {
            //反射机制调用
            $ref = new ReflectionMethod($controller, $func_name);
            $r = Reflection::getModifierNames($ref->getModifiers());
    ...
    public function index($app_name)
    {
         ...
         * 运行控制器
         * ---------------------------------------------------------------------------------------------------*/
        if (false === $this->controller($app_name, $strCtrlor, $strFunc)) {
            self::Show404();
        }
    }
     ...
     * @return boolean
     */
    public function controller($app_name, $ctrl_name, $func_name)
    {
        /* ---------------------------------------------------------------------------------------------------
         * 定义页面输出对象
        * ---------------------------------------------------------------------------------------------------*/
     ...
     * @param bool   $upper_case 大驼峰还是小驼峰
     *
     * @return string 转化后的字符串
     */
    public static function StrCamelize($str, $upper_case = true)
    {
        $str = strtolower($str);
        $arr = explode('_', $str);
     ...
     */
    public static function GetDB($db_full_name)
    {
        if ('' == $db_full_name) {
            return null;
        }
        if (false === strpos($db_full_name, '\\')) {
        ...
        if (is_null(self::GetInstance()->_db_link)) {
            self::GetInstance()->_db_link = array();
        } elseif (isset(self::GetInstance()->_db_link[$db_full_name])) {
    ...
    public static function Debug()
    {
        ...
        foreach (debug_backtrace() as $key => $debug) {
            ...
            //         if (is_object($_args)) {
            //             array_push($args, 'instance of '.get_class($_args));
            //         } else {
